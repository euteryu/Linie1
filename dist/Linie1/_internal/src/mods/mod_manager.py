...
class ModManager:
    ...
    def __init__(self):
        if hasattr(self, '_is_new') and self._is_new:
            # The mods directory is now found using the reliable helper
            self.mods_directory = resource_path('src/mods')
    ...
    def discover_mods(self):
        ...
        # This logic ensures the root of the project (or bundle) is on the path
        # so that `import src.mods...` can work.
        try:
            # For PyInstaller, _MEIPASS is the temp dir.
            project_root = sys._MEIPASS
        except AttributeError:
            # In dev, find the project root by going up from this file's location.
        ...
        if not os.path.exists(mods_dir_absolute):
            print(f"Mod directory not found: {mods_dir_absolute}")
            return
        ...
        for mod_name in os.listdir(mods_dir_absolute):
            ...
            if os.path.isdir(mod_path) and os.path.exists(os.path.join(mod_path, "__init__.py")):
                try:
                    ...
                    # The import path must now be absolute from the project root
                    full_module_path = f"src.mods.{mod_name}.{main_module_name}"
