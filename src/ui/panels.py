...
class GameInfoPanel(IUIComponent):
    ...
    def __init__(self, screen, theme, layout: LayoutConstants):
    ...
    def draw(self, game, current_state):
        ...
        draw_text(self.screen, turn_text, self.layout.UI_TEXT_X, self.layout.UI_TURN_INFO_Y, self.theme["colors"]["text_light"])
        ...
        action_text = f"Actions: {actions_to_display}/{game.MAX_PLAYER_ACTIONS}"
        font = get_font(24)
        action_surf = font.render(action_text, True, self.theme["colors"]["text_light"])
        action_x = self.layout.UI_PANEL_X + self.layout.UI_PANEL_WIDTH - action_surf.get_width() - 15
        self.screen.blit(action_surf, (action_x, self.layout.UI_TURN_INFO_Y))
...
class PlayerInfoPanel(IUIComponent):
    ...
    def draw(self, game, current_state):
        ...
        draw_text(self.screen, line_info, self.layout.UI_TEXT_X, self.layout.UI_ROUTE_INFO_Y, self.theme["colors"]["text_light"])
        draw_text(self.screen, stops_info, self.layout.UI_TEXT_X, self.layout.UI_ROUTE_INFO_Y + self.layout.UI_LINE_HEIGHT, self.theme["colors"]["text_light"])

class HandPanel(IUIComponent):
    """Displays the player's hand of tiles."""
    def __init__(self, screen, strategy_surfaces, pretty_surfaces, theme, layout: LayoutConstants):
        super().__init__(screen)
    ...
    def draw(self, game, current_state):
        ...
        draw_text(self.screen, f"Player {player.player_id}'s Hand:", self.layout.HAND_AREA_X, self.layout.UI_HAND_TITLE_Y, self.theme["colors"]["text_light"])
        ...
        y_pos = self.layout.HAND_AREA_Y
        for i, tile_type in enumerate(player.hand):
            if i >= self.layout.HAND_TILE_COUNT: break
            rect = pygame.Rect(self.layout.HAND_AREA_X, y_pos, self.layout.HAND_TILE_SIZE, self.layout.HAND_TILE_SIZE)
            ...
            if current_state.scene.strategy_view_active:
                pygame.draw.rect(self.screen, self.theme["colors"]["text_light"], rect)
            else:
                pygame.draw.rect(self.screen, self.theme["colors"]["panel_bg"], rect)
            ...
            y_pos += self.layout.HAND_TILE_SIZE + self.layout.HAND_SPACING
...
class MessagePanel(IUIComponent):
    ...
    def draw(self, game, current_state):
        ...
        lower_ui_start_y = self.layout.UI_MESSAGE_Y
        if isinstance(current_state, LayingTrackState):
            if current_state.scene.debug_mode:
                num_rows = (len(current_state.scene.debug_tile_types) + self.layout.DEBUG_TILES_PER_ROW - 1) // self.layout.DEBUG_TILES_PER_ROW
                lower_ui_start_y = self.layout.DEBUG_PANEL_Y + num_rows * (self.layout.DEBUG_TILE_SIZE + self.layout.DEBUG_TILE_SPACING) + 10
            else:
                lower_ui_start_y = self.layout.UI_SELECTED_TILE_Y
        
        draw_text(self.screen, f"Msg: {message}", self.layout.UI_TEXT_X, lower_ui_start_y, self.theme["colors"]["text_light"])
        ...
        draw_text(self.screen, instr_text, self.layout.UI_TEXT_X, lower_ui_start_y + self.layout.UI_LINE_HEIGHT, self.theme["colors"]["text_muted"], size=18)
...
class ButtonPanel(IUIComponent):
    ...
    def __init__(self, screen, theme, layout: LayoutConstants):
        ...
        btn_w, btn_h, btn_s = self.layout.BUTTON_WIDTH, self.layout.BUTTON_HEIGHT, self.layout.BUTTON_SPACING
        btn_y = self.layout.UI_PANEL_Y + self.layout.UI_PANEL_HEIGHT - btn_h - self.layout.BUTTON_MARGIN_Y
        btn_x_start = self.layout.UI_PANEL_X + self.layout.BUTTON_MARGIN_X
        
        self.strategy_view_rect = pygame.Rect(btn_x_start, btn_y, btn_w, btn_h)
        ...
        self.debug_rect = pygame.Rect(self.layout.DEBUG_BUTTON_X, self.layout.DEBUG_BUTTON_Y, self.layout.DEBUG_BUTTON_WIDTH, self.layout.DEBUG_BUTTON_HEIGHT)
        heatmap_y = self.redo_rect.y - btn_h - btn_s
        self.heatmap_rect = pygame.Rect(btn_x_start, heatmap_y, btn_w * 2 + btn_s, btn_h)

    def draw(self, game, current_state):
        strat_on = current_state.scene.strategy_view_active
        self._draw_button(self.strategy_view_rect, f"View: {'Strategy' if strat_on else 'Artistic'}", True, active_color=self.theme["colors"]["positive"] if strat_on else self.theme["colors"]["accent"])
    ...
    def handle_event(self, event, game, current_state):
        if event.type == pygame.KEYDOWN and event.key == pygame.K_h:
            current_state.toggle_hint_action(); return True
        if event.type != pygame.MOUSEBUTTONDOWN or event.button != 1: return False
        mouse_pos = event.pos
        if self.strategy_view_rect.collidepoint(mouse_pos): current_state.toggle_strategy_view_action(); return True
